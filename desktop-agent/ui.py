...
class MainWindow(QMainWindow):
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addWidget(_fl("[F4] Susturma  ·  [F11] ;TamEkran"))
        lay.addStretch()
        lay.addWidget(_fl("FıratMakes Industries  ·  MARK XXXIX  ·  CLASSIFIED"))
        lay.addStretch()
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            has_gemini = bool((d.get("gemini_api_key") or "").strip())
            has_os = bool((d.get("os_system") or "").strip())
            has_mistral = bool((d.get("mistral_api_key") or "").strip()) or bool(
                (d.get("mistral_api_key_secondary") or "").strip()
            )
            return has_gemini and has_os and has_mistral
        except Exception:
            return False
