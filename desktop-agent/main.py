...
from bridge.health_server import start_health_server
...
def main():
    # Load port from config/bridge.json if it exists
    port = 8765
    try:
        config_path = BASE_DIR / "config" / "bridge.json"
        if config_path.exists():
            with open(config_path, "r", encoding="utf-8") as f:
                port = json.load(f).get("health_port", 8765)
    except Exception as e:
        print(f"[JARVIS] ⚠️ Failed to load bridge config port: {e}")
